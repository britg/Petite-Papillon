...
@implementation AddButterflyPhotoViewController
...
-(void) showPicker {
	DebugLog(@"Showing picker for Butterflies");
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
	...
	// determine if that butterfly has a directory
	NSString *pathToButterfly = [pathToDocuments stringByAppendingPathComponent:[NSString stringWithFormat:@"butterflies/%i", self.rowID]];
	NSString *pathToButterflyThumbs = [pathToDocuments stringByAppendingPathComponent:[NSString stringWithFormat:@"butterfly_thumbs/%i", self.rowID]];
	BOOL isDir = YES;
	
	NSFileManager *fileManager = [NSFileManager defaultManager];
	if (![fileManager fileExistsAtPath:pathToButterfly isDirectory:&isDir]) {
		[fileManager createDirectoryAtPath:pathToButterfly withIntermediateDirectories:NO attributes:nil error:nil];
	}
	...
	NSString *pathComponent = [NSString stringWithFormat:@"butterflies/%i/%@.png", self.rowID, [now description]]; 
	...
	// Create thumb image
	NSString *fullPathToThumbImage = [pathToDocuments stringByAppendingPathComponent:[NSString stringWithFormat:@"butterfly_thumbs/%i/%@.png", self.rowID, [now description]]];
	...
	//DebugLog(@"butterfly thumbnail is %@", thumbnail);
	UIGraphicsEndImageContext();
	NSData *imageData = UIImagePNGRepresentation(thumbnail);
	//DebugLog(@"butterfly thumb image data is %@", imageData);
